...
class CommandInterceptor:
    ...
    def _get_tts_speaker(self, source_entity_id: str = None) -> Tuple[str, str, str]:
        """获取TTS音箱信息，返回 (play_entity_id, execute_text_service, play_text_service)"""
        speaker = None
        if source_entity_id:
        ...
        if speaker:
            play_entity = speaker.play_entity_id if speaker.play_entity_id else speaker.entity_id
            return (
                play_entity,
        ...
        if action == "turn_on":
            ...
            if success:
                if mode:
                    ...
                    tts_msg = f"好的，已为你打开{device_name}，{mode_text}模式"
                    notify_msg = f"已经打开{device_name}，模式{mode_text}"
                    if temperature:
                        await self.ha_client.set_ac_temperature(entity_id, temperature)
                        tts_msg += f"，温度{temperature}度"
                        notify_msg += f"，温度{temperature}度"
                    messages.append(tts_msg)
                else:
                    messages.append(f"好的，已为你打开{device_name}")
                    notify_msg = f"已经打开{device_name}"
                    ...
                    if state:
                        ...
                        if current_mode and current_mode != "off":
                            ...
                            notify_msg += f"，模式{mode_text}"
                        if current_temp:
                            notify_msg += f"，温度{current_temp}度"

        elif action == "turn_off":
            success = await self.ha_client.turn_off_ac(entity_id)
            if success:
                messages.append(f"好的，已为你关闭{device_name}")
                notify_msg = f"已经关闭{device_name}"
            ...
            mode_map_dict = {"cool": "制冷", "heat": "制热", "dry": "除湿", "auto": "自动", "fan_only": "送风"}
            tts_parts = [f"好的，已为你设置{device_name}"]
            notify_parts = [f"已经设置{device_name}"]
            
            if temperature is not None:
                success = await self.ha_client.set_ac_temperature(entity_id, temperature)
                if success:
                    tts_parts.append(f"温度{temperature}度")
                    notify_parts.append(f"温度{temperature}度")
            if mode is not None:
                success = await self.ha_client.set_ac_mode(entity_id, mode)
                if success:
                    mode_text = mode_map_dict.get(mode, mode)
                    tts_parts.append(f"{mode_text}模式")
                    notify_parts.append(f"模式{mode_text}")
    ...
    async def _execute_vacuum(self, entity_id: str, action: str, device_name: str, room: str = None, segment_id: int = None, clean_mode: str = None, repeats: int = 1):
        ...
        if action == "clean_segment" and segment_id is not None:
            ...
            if success:
                messages.append(f"好的，{device_name}已停止")
                notify_msg = f"{device_name}已停止"
            ...
            if success:
                messages.append(f"好的，{device_name}已暂停")
                notify_msg = f"{device_name}已暂停"
        elif action == "return_to_base":
            success = await self.ha_client.vacuum_return_to_base(entity_id)
            if success:
                messages.append(f"好的，{device_name}开始回充")
                notify_msg = f"{device_name}开始回充"
    ...
    async def _execute_appliance(self, entity_id: str, action: str, device_name: str, device_type: str):
        ...
        if action == "query":
            ...
            success = await self.ha_client.generic_turn_on(domain, entity_id)
            if success:
                messages.append(f"好的，已为你启动{device_name}")
                notify_msg = f"已经启动{device_name}"
        elif action == "turn_off":
            success = await self.ha_client.generic_turn_off(domain, entity_id)
            if success:
                messages.append(f"好的，已为你停止{device_name}")
                notify_msg = f"已经停止{device_name}"
    ...
    def _format_appliance_state(self, device_name: str, device_type: str, state: str, attributes: Dict[str, Any]) -> str:
        ...
        if device_type == "refrigerator":
            ...
            if temp is not None:
                parts.append(f"当前温度{temp}度")
            if freezer_temp is not None:
                parts.append(f"冷冻室{freezer_temp}度")
            if target_temp is not None:
                parts.append(f"设置温度{target_temp}度")
        ...
        if device_type in ("dishwasher", "washing_machine", "dryer"):
            ...
            if state_lower in ("on", "running", "cleaning", "washing", "drying"):
                parts.append("正在运行")
                if program:
                    parts.append(f"当前程序: {program}")
                if remaining_time is not None:
                    try:
                        mins = int(float(remaining_time))
                        if mins > 0:
                            parts.append(f"还剩{mins}分钟")
                        else:
                            parts.append("即将完成")
                    except (ValueError, TypeError):
                        parts.append(f"剩余时间: {remaining_time}")
                if progress is not None:
                    try:
                        pct = int(float(progress))
                        parts.append(f"已完成{pct}%")
                    except (ValueError, TypeError):
                        pass
    ...
    async def check_cleaning_tasks(self):
        ...
        for entity_id, task in list(self.active_cleaning_tasks.items()):
            ...
            if prev_state in cleaning_states and current_state in finished_states:
                ...
                mins = duration // 60
                secs = duration % 60
                time_text = f"{mins}分{secs}秒" if mins > 0 else f"{secs}秒"
                battery = state.get("attributes", {}).get("battery_level", "")
                
                title = "🤖 扫地机器人清扫完成"
                message = f"{task['device_name']}完成{task['task_desc']}，用时{time_text}"
                if battery:
                    message += f"，当前电量{battery}%"
                
                logger.info(f"清扫任务完成: {message}")
                ...
                if self.config.tts.enabled and speaker_ids:
                    for speaker_entity_id in speaker_ids:
                        play_entity_id = speaker_entity_id
                        play_svc = None
                        for sp in self.config.xiaomi_speakers:
                            if sp.entity_id == speaker_entity_id:
                                play_entity_id = sp.play_entity_id if sp.play_entity_id else speaker_entity_id
                                play_svc = getattr(sp, 'play_text_service', None)
                                break
                        try:
                            await self.ha_client.play_text(play_entity_id, message, service=play_svc)
                        except Exception:
                            pass
